...
class IFU(physicalVersion: Boolean) extends Module {
  ...
  // State 2
  io.toICache.valid := ((ifuState === IFUState.sRequest) || updatePC) && !reset.asBool
  io.toICache.bits.pc := Mux(updatePC, nextPC, pc)
      ...
      when(io.fromICache.fire && !reset.asBool) {
        ifuState := {
          if (physicalVersion) IFUState.sSend
            ...
            Mux(
              io.toIDU.fire,
              Mux(updatePC, IFUState.sRequest, IFUState.sIdle),
              IFUState.sSend
            )
        }
      }
