...
static void decode_operand(Decode *s, int *rd, int *csr_idx, word_t *src1,
                           word_t *src2, word_t *imm, int type) {
  uint32_t i = s->isa.inst.val;
  int rs1 = BITS(i, 19, 15);
  int rs2 = BITS(i, 24, 20);
  *csr_idx = BITS(i, 31, 20);
  *rd = BITS(i, 11, 7);
  switch (type) {
  case TYPE_R:
    src1R();
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */)                   \
  {                                                                            \
    decode_operand(s, &rd, &csr_idx, &src1, &src2, &imm, concat(TYPE_, type)); \
    __VA_ARGS__;                                                               \
  }
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal, J, R(rd) = s->pc + 4,
          s->dnpc = s->pc + imm, ftrace_check(rd, s->dnpc, 1));
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr, I,
          s->dnpc = (src1 + imm) & ~1, R(rd) = s->pc + 4,
          ftrace_check(rd, s->dnpc, src1));
  INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq, B,
          s->dnpc = src1 == src2 ? s->pc + imm : s->pc + 4);
  INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne, B,
          s->dnpc = src1 != src2 ? s->pc + imm : s->pc + 4);
  INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt, B,
          s->dnpc = (int32_t)src1 < (int32_t)src2 ? s->pc + imm : s->pc + 4);
  ...
  INSTPAT("??????? ????? ????? 000 ????? 00000 11", lb, I,
          R(rd) = (int8_t)Mr(src1 + imm, 1));
  INSTPAT("??????? ????? ????? 001 ????? 00000 11", lh, I,
          R(rd) = (int16_t)Mr(src1 + imm, 2)); // sign-extended.
  ...
  INSTPAT("0000000 ????? ????? 011 ????? 01100 11", sltu, R,
          R(rd) = src1 < src2); // Is good when x0, src2 == zero?
  ...
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw, I,
          R(rd) = SR(csr_idx), SR(csr_idx) = src1);
  INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs, I,
          R(rd) = SR(csr_idx), SR(csr_idx) = SR(csr_idx) | src1);
  INSTPAT("0011000 00010 00000 000 00000 11100 11", mret, I,
          s->dnpc = SR(0x341), SR(0x300) &= ~0x8,
          SR(0x300) |= ((SR(0x300) >> 7) & 0x1) << 3, SR(0x300) |= 0x1 << 7);
